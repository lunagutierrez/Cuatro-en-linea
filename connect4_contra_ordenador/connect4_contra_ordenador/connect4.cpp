...
void Linea4::move(int b[ROWS][COL],int col, int player){
  ...
  for (int i = 0; i < ROWS; i++) {
		if(!isColumnFull(col)){
			if (b[i][col] == 0) { // Lo pone en la fila disponible
				b[i][col] = player; // posiciona la pieza
				break;
			}
		}
	}
...
bool Linea4::won(int p) { //recibe tablero y jugador
	int aligned = 0; //contador de piezas contiguas
  bool num_found = false;
//horizontales
	for (int c = 0; c < COL - 3; c++) { // for cada columna sin salirnos del rango
		for (int r = 0; r < ROWS; r++) { // for cada fila
			for (int i = 0; i < 4; i++) { // se necesitan 4
				if (board[r][c + i] == p) {
					aligned++; //sumo
				}
...
// diagonales
	...
	return num_found; //si no, nadie ha ganado
}
